...
- **asset_feed_spec s IG placementy vyžaduje `instagram_user_id` v `object_story_spec`**, jinak `error_subcode 1772103` „Instagram account is missing" (jednoduché object_story_spec-only kreativy IG nevyžadují). ID: `GET /{page_id}?fields=instagram_business_account`. **(živě, 2026)**
